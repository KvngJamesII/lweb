...
const userWarns = {};
let BOT_OWNER = process.env.BOT_OWNER || null; // Set from env or auto-detected from pairing
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (qr) {
      if (process.env.BOT_OWNER) {
        console.log("Auth credentials not found. Please pair again via the web interface.");
        process.exit(1);
        return;
      }
      console.clear();
